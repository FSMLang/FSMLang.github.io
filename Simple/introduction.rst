...
FSMLang can create four different layouts for the event, state, action, and transition data.  The default (or ``-tc`` option) is to simply
...
a switch for the current state.  The final approach (``-tss``) creates a single switch statement for the machine, forming the switch operand by
shifting the event's value to the left and adding the state's.  As usual, the trade-offs between these three approaches involve size and speed.

We'll look at the code generated by each of the four, starting with the parts that are common.
...
The next variant offers the chance of a bit of efficiency when an event is handled identically in all states, and has no associated transition.
...
The fourth implmentation has no need of arrays or suborniate functions.  Instead, all event and state data are combined into a single switch
statement created directly in the main FSM function.  The combination is facilitated by this macro, which, of course, assumes that the machine
does not have an outrageous number of either events or states.
...
This is wrapped in the same while loop as the other implementations.

Depending on the action density in the machine, this implementation will often be the smallest of the four.
